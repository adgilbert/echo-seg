...
from transformation.CycleGAN_and_pix2pix.models.networks import get_norm_layer, init_net
from .camus_unet1 import UNet1
from .camus_unet2 import UNet2
...
def get_network(input_nc, output_nc, filters, model, norm='batch', use_dropout=False, init_type='normal',
    ...
    if model == 'resnet_9blocks':
        ...
        net = UnetGenerator(input_nc, output_nc, 8, filters, norm_layer=norm_layer, use_dropout=use_dropout,
                            include_coordconv=coord_conv)
    elif model == "camus_unet1":
        net = UNet1(input_nc, output_nc)
    elif model == "camus_unet2":
        net = UNet2(input_nc, output_nc)
    # elif model == "sr_hg":
    #     net = StackedRecurrentHourglass(output_nc, use_instance_seg=False, pretrained=False,
    #              use_coordinates=True, usegpu=True)
    # elif model == "unet1_test":
